...
export class ChatMockApi {
  ...
  constructor(private _fuseMockApiService: FuseMockApiService) {
    ...
    this._chats = this._chats.map(chat => ({
      ...
      messages: this._messages.map(message => ({
        ...message,
        chatId: chat.id,
        contactId: message.contactId === 'me' ?
            this._profile.id :
            chat.contactId,
  ...
  registerHandlers(): void {
    ...
    // @ Chat - PATCH
    // -----------------------------------------------------------------------------------------------------
    this._fuseMockApiService.onPatch('api/apps/chat/chat').
        reply(({request}) => {
    ...
    this._fuseMockApiService.onGet('api/apps/chat/contacts').reply(() => {
      ...
      contacts = contacts.map(
          contact => omit(contact, ['details', 'attachments']));
    ...
    // @ Contact Details - GET
    // -----------------------------------------------------------------------------------------------------
    this._fuseMockApiService.onGet('api/apps/chat/contact').
        reply(({request}) => {
